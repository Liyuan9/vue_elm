...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            // 监听cartList变化，更新当前商铺的购物车信息shopCart
            shopCart: function () {
                return Object.assign({}, this.cartList[this.shopId]);
            },
            // shopCart变化时重新计算当前商品数量
            foodNum: function () {
                let category_id = this.foods.category_id;
                let item_id = this.foods.item_id;
                if (this.shopCart && this.shopCart[category_id] && this.shopCart[category_id][item_id]) {
                    let num = 0;
                    Object.value(this.shopCart[category_id][item_id]).forEach((item, index) => {
                        num += item.num;
                    })
                    return num;
                } else {
        ...
        methods: {
            ...
            // 显示规格列表,用$emit调用父组件的showChooseList方法
